function get_current_temp () {
    const requestURL = 'https://api.openweathermap.org/data/2.5/weather?id=5585010&appid=b08bcb8d94b58f5479134fc91470c543&units=imperial';
    fetch(requestURL)
...
function get_forecast() {
    const requestURL2 = 'https://api.openweathermap.org/data/2.5/forecast?id=5585010&appid=b08bcb8d94b58f5479134fc91470c543&units=imperial';
    fetch(requestURL2)
    .then( response => { return response.json(); } )
    .then(  jsonObject => {  
...
function getEvents() {
    ...
    for (i=0; i<towns[2].events.length; i++) {
        ...
        p.textContent = towns[2].events[i]
        card.appendChild(p);
